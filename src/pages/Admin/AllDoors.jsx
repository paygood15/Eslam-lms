...
import { fetchDoors, deleteDoor } from "../../store/DoorSlice";
...
const AllDoors = () => {
  ...
  const handleDeleteConfirm = () => {
    if (courseToDelete) {
      dispatch(deleteDoor(courseToDelete._id)).then((response) => {
        if (response.meta.requestStatus === "fulfilled") {
          toast.success("door deleted successfully!");
          setIsPopupOpen(false);
          setCourseToDelete(null);
          dispatch(fetchDoors()); // Use fetchDoors() to reload the data
        } else {
          toast.error("Failed to delete door. Please try again.");
        }
      });
    }
  };
  ...
  const filteredCourses = doors.data?.filter((course) => {
    // Match search term
    const matchesSearch =
  ...
  return (
    <div className="ltr p-4">
       <div className="main-title flex justify-between items-center p-2">
            <h3 className="text-[18px] font-bold">All Doors</h3>
            <p className="text-[#67748E]">Pages / All Doors</p>
          </div>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr>
              ...
              <th className="p-2 text-left text-[#4A5568]">Lessons</th>
              <th className="p-2 text-left text-[#4A5568]">SubCategory</th>
            ...
            {filteredCourses?.map((course, index) => (
              ...
              >
                ...
                <td className="border-t border-b p-2 flex gap-[12px] items-center justify-evenly">
                  <FaEdit onClick={()=> {
                    navigate(`/Admin/EditDoors/${course._id}`)
