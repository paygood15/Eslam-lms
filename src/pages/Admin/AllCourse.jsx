...
import { fetchCourses, deleteCourse } from "../../store/CourseAdminSlice";
...
console.log(courses);
  ...
  const handleDeleteConfirm = () => {
    if (courseToDelete) {
      dispatch(deleteCourse(courseToDelete._id)).then((response) => {
  ...
  return (
    <div className="ltr p-4">
       <div className="main-title flex justify-between items-center p-2">
            <h3 className="text-[18px] font-bold">All Course</h3>
            <p className="text-[#67748E]">Pages / All Course</p>
          </div>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr>
              ...
              <th className="p-2 text-left text-[#4A5568]">Price</th>
              <th className="p-2 text-left text-[#4A5568]">Total Hours</th>
              <th className="p-2 text-left text-[#4A5568]">Total Questions</th>
