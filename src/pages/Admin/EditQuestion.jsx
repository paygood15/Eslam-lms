...
import { updateQuestion, getSinglequestationAdmin } from "../../store/ExamSlice1";
...
const EditQuestion = () => {
  ...
  const { loading, exams, questAdmin} = useSelector((state) => state.ExamSlice1);
  console.log(exams);
console.log(id);
console.log(examId);
console.log(questAdmin);
  useEffect(() => {
    if (!token || userRole !== "admin") {
      navigate("/");
    } else {
      dispatch(getSinglequestationAdmin({examId,questionId:id}));
    }
  ...
  const handleSubmit = async (e) => {
    ...
    questionData.append("options", JSON.stringify([option1, option2, option3]));
    if (image) questionData.append("image", image);
    questionData.append("score", score);

    try {
      await dispatch(updateQuestion({ examId,questionId:id, questionData })).unwrap();
